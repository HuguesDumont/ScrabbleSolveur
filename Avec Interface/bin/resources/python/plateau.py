...
#orientation est un bool pour savoir si la postion est verticale ou horizontale
	def ajouterMot(self,mot,positionX,positionY,orientation):
	...
	def placementPremierMot(self,listMot):
		possibilite=[]
		i=0
		for mot in listMot:
			i=0
				...
				if self.cases[ligne][i].lettre.valeur=='_': #On cherche jusqu'a tomber sur une lettre
					...
					if all(l in mot for l in (lettresPlat+joueur.getChevalet())): #Si les lettres placées sont dans le mot cherche ou le placer
						...
						for char in mot:
							...
							idx += 1
						for index in indexs:
							if (all(item.lettre.valeur=='_' for item in self.cases[ligne][i-1-index-len(lettresPlat):i-len(lettresPlat)]) and ((i-1-index-len(lettresPlat))>0) or (i-index-len(lettresPlat)==0)):#Peut on placer le mot avant: Si toutes les cases sont vides ou si il est coll� au bord
								if (i-1+len(mot)-index-len(lettresPlat)<15):#Si le mot n'est pas trop grand
									if all(item.lettre.valeur=='_' for item in self.cases[ligne][i:i+len(mot)-index-len(lettresPlat)]): #Si il y a assez de cases vides apr�s pour placer le mot
	...
	def rechercheLettreLigne(self,ligne):
		i=0
	...
	def rechercheLettreColonne(self,colonne):
		i=0
	...
	def AjouterMot(self,mot,joueur : Joueur = None):
		listeCases=[]
		listeLettre=[]
		...
		if mot[3]: #Place en ligne
			for idx,lettre in enumerate(mot[0]):
				...
				self.cases[mot[1]][mot[2]+idx].setType("")
			...
			for idx,lettre in enumerate(mot[0]):
				...
				self.cases[mot[1]+idx][mot[2]].setType("")
	...
	def RetirerMotTemp(self,mot,joueur : Joueur = None):
		listeCases=[]
		listeLettre=[]
	...
	def getListeMot(self,mot): #Cherche tout les mots du tableau de plus de 2 lettres
		listeMot=[]
	...
	def getListeMot2(self): #Cherche tout les mots du tableau de plus de 2 lettres
